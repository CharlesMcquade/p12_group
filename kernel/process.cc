...
long Process::execv(const char* fileName, SimpleQueue<const char*> *args, long argc) {
   ...
   //Debug::printf("found it, preparing address space\n");
    ...
    long userESP = 0xeeeeeee0;
    long* stack = (long*)userESP;
    char** argv = (char**)&stack[1];
    memset(argv, 0, sizeof(char*) * argc);
    char* topOfStack = (char*)&stack[1];
    topOfStack += sizeof(char*) * argc + sizeof(char*);
    argv[0] = (char*)topOfStack;
    long sizeOfArg = sizeof(char) * K::strlen(fileName);
    memcpy(topOfStack, fileName, sizeOfArg);
    topOfStack += sizeOfArg + 1;

    for(long i = 1; i < argc; i++) {
    	argv[i] = (char*)topOfStack;
    	const char* arg = args->removeHead();
    	sizeOfArg = sizeof(char) * K::strlen(arg);
    	memcpy(topOfStack, arg, sizeOfArg);
    	topOfStack += sizeOfArg + 1;
    }
    argv[argc] = nullptr;
    stack[0] = argc;
